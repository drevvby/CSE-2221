...
public final class NaturalNumberStaticOps {
    ...
    private static void printWithCommas(NaturalNumber n, SimpleWriter out) {
        ...
        newN.copyFrom(n);
        NaturalNumber thousandsDiv = new NaturalNumber2(1000);
        String nString = n.toString();

        if (nString.length() > 3) {

            NaturalNumber digitsLeftover = new NaturalNumber2();
            digitsLeftover = newN.divide(thousandsDiv);
            printWithCommas(newN, out);
            out.print("," + digitsLeftover);

        } else {

            out.print(newN.toString());
