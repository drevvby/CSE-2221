...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        String combo = "";
        //set the substring to the amount of overlap given as an argument
        String sub = str1.substring(0, str1.length() - overlap);
        //combine the two strings, it doesn't matter what overlap is or if the string
        //even looks like a word, all that matters is the strings combined meet
        //at the specified overlapping point.
        combo = sub.concat(str2);
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        //create new empty set to be made into a copy
        Set<String> copy = new Set2<>();
        //for every string in strSet, add each value to the copy to actually make
        //it a copy
        for (String tempStr : strSet) {
            copy.add(tempStr);
        }
        //new empty set return set is the set of all strings we want removed from
        //strSet
        Set<String> returnSet = new Set2<>();
        //run while copy isn't empty
        while (copy.size() > 0) {
            //remove a random string from copy and assign it to a temporary string
            String tempStr = copy.removeAny();
            //if the given string is a substring of the temp string, add the given string
            //to the returnSet if it doesn't already exist inside of it
            if (str.indexOf(tempStr) >= 0) {
                if (!returnSet.contains(str)) {
                    returnSet.add(tempStr);
                }
                //if the temporary string is a substring of the given string, then
                //add it to returnSet if it's unique. this if and the above if
                //store substrings in a set that will be removed from the total set
            } else if (tempStr.indexOf(str) >= 0) {
                if (!returnSet.contains(str)) {
                    returnSet.add(str);
                }
                //if neither is a substring of the other, add the temporary string to
                //strSet if it's unique
            } else {
        ...
        //add the given string to the set, and remove all of the substrings
        strSet.add(str);
        strSet.remove(returnSet);
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        Set<String> s1 = new Set2<>();

        //while there is input to be read, take each line of the input as
        //a string, and call to the given method, adding the string to the set
        //iff the line is not a substring of another string in the set
        while (!input.atEOS()) {
            String s = input.nextLine();
            addToSetAvoidingSubstrings(s1, s);
        }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        //for every single character of the text, skip line when the character
        //is equal to ~ but otherwise print the text exactly to console
