...
public final class ArraysAndReferences {
    ...
    private static NaturalNumber productOfArrayElements(
        ...
        NaturalNumber prod = new NaturalNumber2(nnArray[0]);

        int i = 0;
        while (i < nnArray.length) {
            prod.multiply(nnArray[i]);
            ++i;
        }
        return prod;
    }
    ...
    private static void computePartialProducts(NaturalNumber[] nnArray) {
        ...
        if (nnArray.length >= 1) {
            while (i < nnArray.length) {
                NaturalNumber temp = new NaturalNumber2(nnArray[i - 1]);
                //nnArray[i] = nnArray[i].mu;
                ++i;
            }
        }
